...
class EVSegLoader(object):
    def __init__(self, data_path, win_size, step, dataset, mode="train"):
        self.mode = mode
        self.step = step
        self.win_size = win_size
        self.scaler = StandardScaler()  # 将数据转换为均值为0，方差为1的分布
        new_path = f"{data_path}/{dataset}_train.npy"
        data = np.load(new_path)
        # 训练数据对 scaler 进行拟合，计算出数据的均值和方差等统计量
        # 然后使用这些统计量对训练数据进行标准化转换，将转换后的数据重新赋值给 data 变量。
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_path = f"{data_path}/{dataset}_test.npy"
        test_data = np.load(test_path)
        self.test = self.scaler.transform(test_data)  # 使用之前在训练数据上拟合好的 scaler 对测试数据进行标准化转换
        self.train = data
        self.val = self.test
        labels_path = f"{data_path}/{dataset}_test_label.npy"
    ...
    def __len__(self):
        if self.mode == "train":
            return (self.train.shape[0] - self.win_size) // self.step + 1  # 计算训练数据按照给定窗口大小和步长划分后可以得到的样本数量
        elif (self.mode == 'val'):
            return (self.val.shape[0] - self.win_size) // self.step + 1
        elif (self.mode == 'test'):
            return (self.test.shape[0] - self.win_size) // self.step + 1
    ...
    # 获取指定索引的样本,使用类似 obj[index] 的方式访问 EVSegLoader 类的实例时，会调用这个方法
    def __getitem__(self, index):
        index = index * self.step
        if self.mode == "train":  # train and val did not use label
            # 返回标准化后的训练数据中从 index 位置开始、长度为 win_size 的数据切片,同时返回测试数据标签中前win_size长度的数据切片
            return np.float32(self.train[index:index + self.win_size]), np.float32(self.test_labels[0:self.win_size])
        elif (self.mode == 'val'):
            return np.float32(self.val[index:index + self.win_size]), np.float32(self.test_labels[0:self.win_size])
        elif (self.mode == 'test'):
...
class SWATSegLoader(Dataset):
    def __init__(self, root_path, win_size, step=1, flag="train"):
        ...
        self.test = test_data
        data_len = len(self.train)
        self.val = self.train[(int)(data_len * 0.8):]  # val占比0.2
...
def get_loader_segment(index, data_path, batch_size, win_size=100, step=100, mode='train', dataset='KDD'):
    ...
    elif (dataset == 'EV24' or dataset == 'EV47' or dataset == 'VLoong'):
        dataset = EVSegLoader(data_path, win_size, 1, dataset, mode)
