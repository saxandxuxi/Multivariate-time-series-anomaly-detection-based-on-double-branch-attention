    ...
    def test(self):
        ...
        for i, (input_data, labels) in enumerate(self.thre_loader):
            ...
            for u in range(len(prior)):
                ...
                # 计算双向KL散度（同训练时的L_P和L_N，但仅用于评分，不反向传播）
                kl_p = my_kl_loss(prior[u], n_normalized.detach())  # KL(P || Stopgrad(N))
                kl_n = my_kl_loss(series[u], p_normalized.detach())  # KL(N || Stopgrad(P))

                # 累加多尺度损失并应用温度缩放
                series_loss += kl_n * temperature
                prior_loss += kl_p * temperature
